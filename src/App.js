...
const styles = {
  ...
  activeBtn: { color: '#FF85A1' },
  plusBtn: { backgroundColor: '#FF85A1', color: '#fff', width: '45px', height: '45px', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '24px', border: 'none', marginBottom: '20px' }
};

function App() {
  const [isLoggedIn, setIsLoggedIn] = useState(false);
  const [activeTab, setActiveTab] = useState('home'); // home, chat, profile
  ...
  const handleLogin = async (e) => {
    ...
    else alert("Login failed! Check your unique password.");
  };
    ...
    return (
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: '100vh', backgroundColor: '#FFF5F7' }}>
        <form onSubmit={handleLogin} style={{ background: '#fff', padding: '30px', borderRadius: '20px', width: '280px', textAlign: 'center' }}>
          ...
          <button style={{ background: '#FF85A1', color: '#fff', border: 'none', padding: '10px', width: '100%', borderRadius: '10px' }}>Login</button>
        </form>
      </div>
    );
  }
    ...
    <div style={styles.container}>
      ...
      {activeTab === 'home' && (
        <div style={styles.feedGrid}>
          ...
          )) : <p style={{ gridColumn: 'span 2', textAlign: 'center', color: '#999' }}>No posts yet. Be the first! ✨</p>}
        </div>
      )}

      {activeTab === 'chat' && (
        <div style={{ flex: 1, padding: '20px' }}>Chat interface goes here...</div>
      )}
